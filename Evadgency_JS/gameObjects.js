  ...
  for (var i = 0; i < laneSpawn.length; i++) {
    ...
    if (temp < 50) {
      ...
      for (var ii = 0; ii < increaseSpawn(gameMaster.difficulty); ii++) {
        for (var i = 0; i < laneSpawn.length; i++) {
          if (rollChance(1) < 50) {
            new gameObject(obstacles, "obstacleRight", "sprites/spritesheet.png", 64, 64 * getRandomInt(5, 8), 64, 64, Math.round(Math.random() * 608), laneSpawn[i], (Math.random() * laneSpeed) + 1, 32, 32);
          } else {
            new gameObject(obstacles, "obstacleLeft", "sprites/spritesheet.png", 64 * 3, 64 * getRandomInt(5, 8), 64, 64, Math.round(Math.random() * 608), laneSpawn[i], (Math.random() * laneSpeed) + 1, 32, 32);
          }
        }
      }
