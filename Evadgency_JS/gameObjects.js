...
export function initObstacles() { //if statements to initialize same code for different rows to create full layout
    ...
    for (var i = 0; i < laneSpawn1.length; i++) {
        ...
        if (temp < 50) {
            new gameObject(obstacles, "obstacleRight", "sprites/spritesheet.png", 64, 64 * computerResponse, 64, 64, Math.round(Math.random() * 576), laneSpawn1[i], Math.floor(Math.random() * 0) + 0, 32, 32);
        } else {
            new gameObject(obstacles, "obstacleLeft", "sprites/spritesheet.png", 64 * 3, 64 * computerResponse, 64, 64, Math.round(Math.random() * 576), laneSpawn1[i], Math.floor(Math.random() * 0) + 0, 32, 32);
        }
